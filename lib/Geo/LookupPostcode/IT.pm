...
sub lookup_it_postcode {
    ...
    my $lookup_key = substr($postcode, 0, 3);

    my @results;
    for my $rh_geo_entry (@{ $lookup_table{$lookup_key} }) {
        local $_ = $postcode;
...
sub _build_data() {
    ...
    %lookup_table = ();
    for my $rh_geo_entry (@geo_provinces) {
        $rh_geo_entry->{region_code} = $geo_regions{$rh_geo_entry->{region}};
        for my $key (@{ $rh_geo_entry->{lookup_keys} }) {
            $lookup_table{$key} //= [];
            push @{ $lookup_table{$key} }, $rh_geo_entry;
        }
    }
...
sub _test_check_lookup_table {
    ...
    require Test::More;
    for my $key (sort keys %lookup_table) {
        if (@{ $lookup_table{$key} } >= 2) {
            if (! _all(sub { $_->{function} }, @{ $lookup_table{$key} })) {
                my $provinces_string = join(", ",
                    map { $_->{province} } @{ $lookup_table{$key} }
                );
                Test::More::fail("Lookup key '$key' in country IT has multiple provinces $provinces_string but missing functions to distinguish between them. ");
            }
        }
    }
}
